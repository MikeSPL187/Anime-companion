...
import 'package:ani_app/features/anime_details/application/anime_details_providers.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:flutter_test/flutter_test.dart';

void main() {
  test(
    'details status and favorite actions persist presentation snapshot',
    () async {
      ...
      final actions = container.read(animeDetailsActionsProvider);
      await actions.setStatus(_summary(), LibraryStatus.watching);
      await actions.toggleFavorite(_summary());

      expect(repository.lastStatusAnimeId, '413');
      expect(repository.lastStatus, LibraryStatus.watching);
      expect(repository.lastStatusSnapshot?.title, 'Наруто');
      expect(repository.lastFavoriteAnimeId, '413');
      expect(repository.lastFavoriteSnapshot?.title, 'Наруто');
      expect(repository.lastFavoriteSnapshot?.episodesTotal, 12);
    },
  );
}
...
class RecordingLibraryRepository implements LibraryRepository {
  ...
  String? lastFavoriteAnimeId;
  LibraryPresentationSnapshot? lastFavoriteSnapshot;
  ...
  Future<void> toggleFavorite(
    ...
    lastFavoriteAnimeId = animeId;
    lastFavoriteSnapshot = snapshot;
  }
